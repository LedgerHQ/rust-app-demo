...
// EVENTS
#define SEPROXYHAL_TAG_SESSION_START_EVENT         0x01 // <kind(1byte)=reqble|recovery|flashback> 
...
#define SEPROXYHAL_TAG_STATUS_EVENT                0x15 // <flags(4BE)> <screen_backlight_percentage(1B)> <ledcolorARGB(4B)> <battery_voltage_mv(4BE)> <battery % (1B)>
...
#define SEPROXYHAL_TAG_MCU_TYPE_PROTECT             0x02 // for instance ask RDP2 to be engaged
#endif // HAVE_MCU_PROTECT
#define SEPROXYHAL_TAG_MCU_BOOTLOADER              SEPROXYHAL_TAG_MCU
#endif // HAVE_SEPROXYHAL_MCU
#define SEPROXYHAL_TAG_UNSEC_CHUNK_READ            0x32 // <length:U2BE> <restart(bit1)|continue=0>
// available if SEPROXYHAL_TAG_SESSION_START_EVENT_FEATURE_ISET_MCUSEC
#define SEPROXYHAL_TAG_UNSEC_CHUNK_READ_EXT        0x33 // <length:U2BE> <continue=0|restart(bit1)|specifyoffset(bit2)> <offset:U4BE>
#define SEPROXYHAL_TAG_BLE_SEND                    0x38 // <opcode(U2BE)> <hci packet payload>
#define SEPROXYHAL_TAG_SET_SCREEN_CONFIG           0x3E // <flags(1byte):pwron(128)|rotation(0:0,90:2,180:4,270:6)|invert(1)> <brightness percentage(1byte)>
#define SEPROXYHAL_TAG_SET_LINK_PROP               0x3F // <mode(1b):debug(1)>
#define SEPROXYHAL_TAG_BLUENRG_SEND                0x40 // <CS mgmt: start (bit 1) | stop (bit 2)> <packet>
...
#define SEPROXYHAL_TAG_USB_CONFIG_ENDPOINTS         0x04  // <nbendpoints to configure(1)> [<epaddr> <eptype:control/interrupt/bulk/isochrone/disabled> <epmps>]
...
#define SEPROXYHAL_TAG_REQUEST_STATUS              0x52 // no args, request power levels of all peripherals and current charging state or not if a battery is present.
#define SEPROXYHAL_TAG_RAPDU                       0x53 // raw response apdu transport
#define SEPROXYHAL_TAG_I2C_XFER                    0x54 // <flags:b0=Start,b1=Stop,b2=Moreexpected> <address+R/w> <write:rawdata,read=length(1B)>
...
//#define SEPROXYHAL_TAG_GENERAL_STATUS_MORE_COMMAND 0x0001 // it's a status, but it shall be a command instead to avoid perturbating the simple seproxyhal bus logic
//#define SEPROXYHAL_TAG_GENERAL_STATUS_ERROR        0x0002 // shall be a command instead
...
#define SEPROXYHAL_TAG_SCREEN_DISPLAY_STATUS       0x65 // <bagl_component_t little endian>
#define SEPROXYHAL_TAG_PRINTF_STATUS               0x66
#define SEPROXYHAL_TAG_SET_LINK_SPEED              0x67 // <mhz(1byte)> <etu(1byte)>
#define SEPROXYHAL_TAG_SCREEN_ANIMATION_STATUS     0x68 // <kind(1byte)> <parameters(variable)> // replied with a display processed event when done
...
#ifdef TARGET_BLUE
#define SEPROXYHAL_TAG_SCREEN_DISPLAY_RAW_STATUS   0x69 // <start:0|next:1> [start? <x> <y> <w> <h> <bitperpixel> <color_count*4 bytes (LE encoding)>] <icon bitmap (row scan, packed, LE)>
