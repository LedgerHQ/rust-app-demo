...
CXCALL void cx_pbkdf2(cx_md_t md_type,
                      const unsigned char WIDE *password PLENGTH(passwordlen), unsigned short passwordlen,
                      unsigned char* salt PLENGTH(saltlen),  unsigned short saltlen,
                      unsigned int iterations,
                      unsigned char* out PLENGTH(outLength), unsigned int outLength);
...
#define cx_pbkdf2_sha512(password, password_len, salt, salt_len, iterations, out, out_len) \
    cx_pbkdf2(CX_SHA512, password, password_len, salt, salt_len, iterations, out, out_len)
