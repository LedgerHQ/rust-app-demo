...
#define UPPER_ALIGN(adr, align, type) (type)((type)((type)(adr)+(type)((type)((type)MAX((type)(align),(type)1UL))-(type)1UL))&(type)(~(type)((type)((type)MAX(((type)align),(type)1UL))-(type)1UL)))
#define LOWER_ALIGN(adr, align, type) ((type)(adr)&(type)((type)~(type)(((type)MAX((type)(align),(type)1UL))-(type)1UL)))
...
#define U4LE_ENCODE(buf, off, value) {(buf)[(off)+3] = ((value)>>24)&0xFF;(buf)[(off)+2] = ((value)>>16)&0xFF;(buf)[(off)+1] = ((value)>>8)&0xFF;(buf)[(off)+0] = ((value))&0xFF;}
#define U2BE_ENCODE(buf, off, value) {(buf)[(off)+0] = ((value)>>8)&0xFF;(buf)[(off)+1] = ((value))&0xFF;}
#define U2LE_ENCODE(buf, off, value) {(buf)[(off)+1] = ((value)>>8)&0xFF;(buf)[(off)+0] = ((value))&0xFF;}

/**
 * Helper to perform compilation time assertions
 */
#if ! defined(SYSCALL_GENERATE) && (defined(__clang__) || (__GNUC__ > 4 || (__GNUC__ == 4 && (__GNUC_MINOR__ >= 6))) )
...
#define __xx_CCASSERT_LINE(predicate, file, line) typedef char CCASSERT_ ## file ## _line_## line [((predicate)?1:-1)]
#endif
...
// function that align the deferenced value in a rom struct to use it depending on the execution address
// can be used even if code is executing at the same place where it had been linked
#ifndef PIC
...
/**
 * The application is considered an IO task by the system. It only gives privileges to BOLOS' internal IO task
 */
...
// send tx_len bytes (atr or rapdu) and retrieve the length of the next command apdu (over the requested channel)
 ...
 */
// io callback in the application called when an interrupt based channel has received data to be processed
...
// write in persistent memory, to make things easy keep a layout of the memory in a structure and update fields upon needs
// The function throws exception when the requesting application buffer being written in its declared data segment. The later is declared during the application slot allocation (using --dataSize parameter in the python scripts)
// NOTE: accept copy from far memory to another far memory.
// @param src_adr NULL to fill with 00's
SYSCALL void nvm_write(void WIDE * dst_adr PLENGTH(src_len), void WIDE * src_adr PLENGTH(src_len), unsigned int src_len);
// the priviledged version of nvm_write, called by bolos itself
void nvm_write_os(void WIDE * dst_adr PLENGTH(src_len), void WIDE * src_adr PLENGTH(src_len), unsigned int src_len);
...
typedef enum bolos_ux_e {
  ...
  BOLOS_UX_KEYBOARD,
  BOLOS_UX_WAKE_UP, // the application/os asks for the screen to be waked up (asking pin if the lock period has been exceeded)
  BOLOS_UX_STATUS_BAR,
  
  BOLOS_UX_BOOT, // will never be presented to loaded UX app, this is for failsafe UX only
  ...
  BOLOS_UX_APP_ACTIVITY, // special code when application is processing data but not displaying UI. It prevent going poweroff, while not avoiding screen locking
  BOLOS_UX_CONSENT_NOT_INTERACTIVE_ONBOARD,
  BOLOS_UX_PREPARE_RUN_APP, // called before the os runs an application, the os ux must be cleared and not display anything on upcoming display processed events
  ...
  BOLOS_UX_ASYNCHMODAL_PAIRING_REQUEST, // ask the ux to display a modal to accept/reject the current pairing request
...
/* Value returned by os_ux to notify the application that the processed event must be discarded and not processed by the application. Generally due to handling of power management/dim/locking */
...
#define BOLOS_TAG_DATA_SIZE 0x05 // meta tag to retrieve the size of the data section for the application
// Library Dependencies are a tuple of 2 LV, the lib appname and the lib version (only exact for now). When lib version is not specified, it is not check, only name is asserted
// The DEPENDENCY tag may have several occurences, one for each dependency (by name). Malformed (multiple dep to the same lib with different version is is not ORed but ANDed, and then considered bogus)
...
typedef struct application_s {
  ...
  // nvram stop address (exclusive) for this application (to check overlap when loading, and mpu lock)
...
typedef struct bolos_ux_params_s {
  ...
  union {
    ...
    // pass the whole load application to that the os version and hash are available
    struct {
      unsigned int app_idx;
...
//SYSCALL PERMISSION(APPLICATION_FLAG_BOLOS_UX) void           os_perso_set_alternate_pin(unsigned char* pin PLENGTH(pinLength), unsigned int pinLength); 
//SYSCALL PERMISSION(APPLICATION_FLAG_BOLOS_UX) void           os_perso_set_alternate_seed(unsigned char* seed PLENGTH(seedLength), unsigned int seedLength);
 ...
 */
SYSCALL                                       unsigned int   os_perso_seed_cookie(unsigned char * seed_cookie PLENGTH(seed_cookie_length), unsigned int seed_cookie_length);


// endorsement APIs
SYSCALL                                       unsigned int   os_endorsement_get_code_hash(unsigned char* buffer PLENGTH(32));
SYSCALL                                       unsigned int   os_endorsement_get_public_key(unsigned char index, unsigned char* buffer PLENGTH(65));
SYSCALL                                       unsigned int   os_endorsement_get_public_key_certificate(unsigned char index, unsigned char* buffer PLENGTH(1 +1 +2*(1+1+33)));
SYSCALL                                       unsigned int   os_endorsement_key1_get_app_secret(unsigned char* buffer PLENGTH(64));
SYSCALL                                       unsigned int   os_endorsement_key1_sign_data(unsigned char* src PLENGTH(srcLength), unsigned int srcLength, unsigned char* signature PLENGTH(1 +1 +2*(1+1+33)));
SYSCALL                                       unsigned int   os_endorsement_key2_derive_sign_data(unsigned char* src PLENGTH(srcLength), unsigned int srcLength, unsigned char* signature PLENGTH(1 +1 +2*(1+1+33)));


// nvram shared zone access right => MPU opening at application switch, using a flags in the registry
...
SYSCALL PERMISSION(APPLICATION_FLAG_BOLOS_UX) unsigned int   os_registry_count(void);
// return any entry, activated or not, enabled or not, empty or not. to enable full control
SYSCALL PERMISSION(APPLICATION_FLAG_BOLOS_UX) void           os_registry_get(unsigned int index, application_t* out_application_entry PLENGTH(sizeof(application_t)));


// return !0 when ux scenario has ended, else 0
// while not ended, all display/touch/ticker and other events are to be processed by the ux. only io is not processed.
// when returning !0 the application must send a general status (or continue its command flow)
...
// Read the last os_ux call parameters from within the UX app. to make easier the firewalling between tasks.
SYSCALL PERMISSION(APPLICATION_FLAG_BOLOS_UX) void os_ux_read_parameters(bolos_ux_params_t* params PLENGTH(sizeof(bolos_ux_params_t)));
...
// syscall test
//SYSCALL void dummy_1(unsigned int* p PLENGTH(2+len+15+ len + 16 + sizeof(io_send_t) + 1 ), unsigned int len);
...
// read from the application's install parameter TLV (if present), else 0 is returned (no exception thrown). 
 ...
 */
SYSCALL PERMISSION(APPLICATION_FLAG_BOLOS_UX) unsigned int os_registry_get_tag(unsigned int appidx, unsigned int* tlvoffset, unsigned int tag, unsigned int value_offset, void* buffer PLENGTH(maxlength), unsigned int maxlength);

// Copy the currently running application tag from its install parameters to the given user buffer.
// Only APPNAME/APPVERSION/DERIVEPATH/ICON tags are retrievable to avoid install parameters private information.
// Warning: this function returns tag content of the application lastly scheduled to run, not the tag content of the currently executed piece of code (libraries subcalls)
...
SYSCALL unsigned int os_customca_verify(unsigned char* hash PLENGTH(32), unsigned char* sign PLENGTH(sign_length), unsigned int sign_length);
...
typedef enum {
  /* Watchdog consumption lead to no action being taken, overflowed value is accounted and can be retrieved by the application */
...
/**
 * This function arm a low level watchdog, when the value is consumed, and depending on the requested behavior, 
 * an action can be taken.
 * @throw INVALID_PARAMETER when the useconds value overflows the possible value.
 */
void os_watchdog_arm(unsigned int useconds, os_watchdog_behavior_t behavior);

/**
 * This function returns the number of useconds to be still consumed by the watchdog (when > 0), or the overflowed useconds after the watchdog has timed out (when < 0)
 */
...
SYSCALL TASKSWITCH PERMISSION(APPLICATION_FLAG_BOLOS_UX) void os_sched_exec(unsigned int application_index);
 ...
 */
SUDOCALL PERMISSION(APPLICATION_FLAG_NONE) bolos_task_status_t os_sched_last_status(unsigned int task_idx);
 ...
 */
SUDOCALL TASKSWITCH PERMISSION(APPLICATION_FLAG_NONE) void os_sched_yield(bolos_task_status_t status);
 ...
 */
SUDOCALL TASKSWITCH PERMISSION(APPLICATION_FLAG_NONE) void os_sched_switch(unsigned int task_idx, bolos_task_status_t status);
 ...
 */
SUDOCALL PERMISSION(APPLICATION_FLAG_NONE) unsigned int os_sched_current_task(void); 
 ...
 * @param permissions to give to the task, permissions is a AND mask with the application's installation flags (can only reduce scope, never grant more).
 ...
 */
SYSCALL void screen_set_keepout(unsigned int x, unsigned int y, unsigned int width, unsigned int height);
/**
 * Draw the given bitmap, with the given colors and position into the screen buffer. Don't update the screen driver.
 */
SYSCALL void bagl_hal_draw_bitmap_within_rect(int x, int y, unsigned int width, unsigned int height, unsigned int color_count, const unsigned int *colors PLENGTH(color_count*4), unsigned int bit_per_pixel, const unsigned char* bitmap PLENGTH(1+bitmap_length_bits/8), unsigned int bitmap_length_bits);
/**
 ...
 * @param speed_and_master enables to set the bus speed. And to select if the peripheral will act as master (issuing Start and Stop condition upon need) or slave mode.
 * @param address In master mode, this parameter sets the target I2C device's address. In slave mode, the address is the desired I2C bus address for the interface. The address is always a 7bit address (excluding the transfer direction bit).
 */
SYSCALL void io_i2c_setmode(unsigned int speed_and_master, unsigned int address);

/**
 * Setup the I2C peripheral for:
 * - In slave mode, receiving a WRITE transaction of maxlength bytes at most. Upon WRITE transaction end, an SEPROXYHAL_TAG_I2C_EVENT is issued with the received data. It has to be received through ::io_seph_recv.
 ...
 * - In slave mode, this call is non-blocking. It only enables to reply to a READ transaction of at most length bytes. After the Stop condition is issued from the master, a SEPROXYHAL_TAG_I2C_EVENT event containing the effectively transferred length is issued and can be retrieved through ::io_seph_recv. To restart or continue the transfer requires another call to ::io_i2c_xfer.
 * - In master mode, this call is blocking and triggers the transaction as requested through the flags parameter. The READ or WRITE transaction will place or transmit data from the given buffer and length. Depending on the passed start/stop flags, corresponding bus condition are executed.
 */
