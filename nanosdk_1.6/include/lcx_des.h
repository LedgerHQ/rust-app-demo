...
/** DES key container.
 *  Such container should be initialize with cx_des_init_key to ensure future API 
 *  compatibility. Indeed, in next API level, the key store format may changed at all.
 *  Only 8 bytes (simple DES) and 16 bytes (triple DES with 2 keys) are supported.
 */
struct cx_des_key_s {
  /** key size */
  unsigned char size;
 ...
 */
CXCALL int cx_des_init_key(const unsigned char WIDE *rawkey PLENGTH(key_len), unsigned int key_len,
...
CXCALL CXPORT(CXPORT_ED_DES) int cx_des_iv(const cx_des_key_t WIDE *key PLENGTH(sizeof(cx_des_key_t)), 
                                           int mode,
 ...
 */
CXCALL CXPORT(CXPORT_ED_DES) int cx_des(const cx_des_key_t WIDE *key PLENGTH(sizeof(cx_des_key_t)),
                                         int mode,
                                         const unsigned char  WIDE *in PLENGTH(in_len), unsigned int in_len, 
                                         unsigned char *out PLENGTH(out_len), unsigned int out_len);
